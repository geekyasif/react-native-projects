    ...
    return (
        <View style={styles.container}>
            ...
            <Fontisto name="injection-syringe" size={24} color="#F1C40F" />
                <Text style={styles.vaccine}>{item.sessions.sessions}</Text>
                <Text style={styles.charge}>{item.sessions.fee_type}</Text>
            </View>
            <View style={styles.row}>
                <Text style={styles.age}>Age Limit: {item.sessions.min_age_limit}</Text>
                <Text style={styles.availablity}>Availablity: {item.sessions.available_capacity}</Text>
            </View>
        </View>
    )
}
